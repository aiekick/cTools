...
void Logger::ShowUI(ImGuiContext *vContext)
{
	ImGuiWindowFlags flags = /*ImGuiWindowFlags_NoDocking |*/ ImGuiWindowFlags_MenuBar;

	auto window = ImGui::FindWindowByName("Console Gui");

	ImGui::Begin("Console Gui", (bool*)0, flags);

	for (auto itFile = m_ConsoleMap.begin(); itFile != m_ConsoleMap.end(); ++itFile)
	{
		if (ImGui::TreeNode(itFile->first.c_str()))
		{
			for (auto itFunc = itFile->second.begin(); itFunc != itFile->second.end(); ++itFunc)
			{
				if (ImGui::TreeNode(itFunc->first.c_str()))
				{
					for (auto itLine = itFunc->second.begin(); itLine != itFunc->second.end(); ++itLine)
					{
						if (ImGui::TreeNode(itLine->first.c_str()))
						{
							for (auto itMsg = itLine->second.begin(); itMsg != itLine->second.end(); ++itMsg)
							{
								ImGui::Selectable((*itMsg).c_str());
							}
							ImGui::TreePop();
						}
					}
